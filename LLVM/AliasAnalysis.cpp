...
//All the added code is marked with INJECT and END INJECT banenrs as seen below.
// The rest of the code in this document is left untouched as the original
// llvm-3.8.0 implementation of AliasAnalysis.

#include "llvm/Analysis/AliasAnalysis.h"

//#include "llvm/Analysis/AliasAnalysis_clone.h"
...
//INJECT-----------------------------------------------------------------------
#include "/home/bartok/ParProj/xDRFPair-src/xDRFPair/xDRFPair.cpp"
//END INJECT-------------------------------------------------------------------
...
//Prepare the object containing the xDRF-Pair data in scope for both implemented
// functions and the pass itself.
//INJECT-------------------------------------------------------------------
xDRFPairData* D;
//END INJECT---------------------------------------------------------------
...
//Using the xDRF-Pair data to modify the getModRefInfo implementations: Simply
// call the isPaied function from the xDRF-Pair data object which returns true
// if the instructions compared are "xDRF-Pair safe" or not.

//The same is true for all the getModRefInfo implementations.
ModRefInfo AAResults::getModRefInfo(Instruction *I, ImmutableCallSite Call) {
  //BEGIN INJECT ---------------------------------------------------------
  llvm::dbgs() << "Used getModRefInfo!\n";
  if(D != NULL){
    if(D->isPaired(I,Call)){
      llvm::dbgs() << "Entered mod-block!\n";
      return MRI_NoModRef;
    }
  }
  //END INJECT -----------------------------------------------------------
...
//This implementation of getModRefInfo has a small exception compared to the
// others, since we still care about 'volatile', the "xDRF-Pair safety" is
// examined after the 'isUnordered' check.
...
bool AAResultsWrapperPass::runOnFunction(Function &F) {
  ...
  //INJECT
  XDRFPair &XDRFP = getAnalysis<XDRFPair>();
  D = XDRFP.ref();
  //END INJECT
  ...
  if (auto *WrapperPass = getAnalysisIfAvailable<TypeBasedAAWrapperPass>())
    AAR->addAAResult(WrapperPass->getResult());
  if (auto *WrapperPass = getAnalysisIfAvailable<objcarc::ObjCARCAAWrapperPass>())
    AAR->addAAResult(WrapperPass->getResult());
